...
# Function to generate a random IPv4 address within a specific range
def random_ip_within_range(start_ip="1.1.1.1", end_ip="254.254.254.254"):
    start_parts = list(map(int, start_ip.split(".")))
    end_parts = list(map(int, end_ip.split(".")))

    while True:
        ip_parts = [random.randint(start_parts[i], end_parts[i]) for i in range(4)]
        ip = ".".join(map(str, ip_parts))
        return ip

# Generate packets related to Jake with the embedded flag
flag = "MAGPIE{aHR0cHM6Ly95b3V0dWJlLmNvbS9zaG9ydHMvV3o0VEtRbVVCQjg=}"  # Define the flag
split_flag = ["MAGPIE{Part1-aHR0cHM6Ly95b3V0dW", "Part2-JlLmNvbS9zaG9ydHMv", "Part3-V3o0VEtRbVVCQjg=}"]

for i in range(50):  # 50 packets linked to Jake
    dst_ip = random_ip_within_range("52.129.50.1", "52.129.50.254")  # Ensure it's in the specified range
    if i == 25:  # Embed the full flag or parts
        # Uncomment below to split flag across multiple packets
        packets.append(create_tcp_packet(jake_ip, dst_ip, random.randint(1024, 65535), 80, payload=split_flag[0]))
        packets.append(create_tcp_packet(jake_ip, dst_ip, random.randint(1024, 65535), 80, payload=split_flag[1]))
        packets.append(create_tcp_packet(jake_ip, dst_ip, random.randint(1024, 65535), 80, payload=split_flag[2]))
    ...
    pkt = create_tcp_packet(jake_ip, dst_ip, random.randint(1024, 65535), 80, payload=f"{clue} {i}")
    packets.append(pkt)
