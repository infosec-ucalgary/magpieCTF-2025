...
public class AuthVerb : IVerb
{
    ...
    public AuthVerb(AuthExtensionProcessor authExtensionProcessor) => AuthExtensionProcessor = authExtensionProcessor;
    ...
    public async Task Process(IConnection connection, SmtpCommand command)
    {
        ...
        if (argumentsParser.Arguments.Count > 0)
        {
            ...
            if (!await AuthExtensionProcessor.IsMechanismEnabled(mechanism).ConfigureAwait(false))
            {
                throw new SmtpServerException(
                    new SmtpResponse(
            ...
            while (status == AuthMechanismProcessorStatus.Continue)
            {
                ...
                {
                    await connection
                        .WriteResponse(new SmtpResponse(StandardSmtpResponseCode.SyntaxErrorInCommandArguments,
                            "Authentication aborted")).ConfigureAwait(false);
                    return;
                }
            ...
            if (status == AuthMechanismProcessorStatus.Success)
            {
                await connection.WriteResponse(new SmtpResponse(
                    ...
                    "Authenticated OK MagpieCTF W4uww_Em41lz_4_eVaR")).ConfigureAwait(false);
