    ...
    public virtual async Task Process(IConnection connection, SmtpCommand command)
    {
        if (connection.CurrentMessage == null)
        {
            await connection.WriteResponse(new SmtpResponse(
                ...
                "Ruh Roh Raggy - Bad sequence of commands")).ConfigureAwait(false);
            return;
        }
        ...
        if (maxMessageSize.HasValue && messageSize > maxMessageSize.Value)
        {
            await connection.WriteResponse(
                new SmtpResponse(
                    ...
                    "Message exceeds fixed size limit. Flags aren't that big you know...")).ConfigureAwait(false);
        ...
        try
        {
            ...
            await connection.WriteResponse(new SmtpResponse(StandardSmtpResponseCode.OK, "Mail Accepted! TY For the Flag <3"))
